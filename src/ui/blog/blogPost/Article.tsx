...
export default function Article({ data }: { data: BlogPost }) {
  const { title, content, author, tags, createdAt, poster } = data;
  const parsedDate = new Date(createdAt);
  return (
    <article className="flex flex-col gap-[32px] z-30 relative h-full max-w-[100vw] overflow-hidden box-border">
      <h1 className="text-[2rem] sm:text-[2.5rem] font-extrabold text-justify">
        {title}
      </h1>
      <div className="flex flex-col gap-[32px] sm:px-[3.75rem] h-full">
        ...
        <div className="tracking-[3%] leading-[210%] text-justify text-[1.25rem] font-medium">
          <Image
            src={`${BASE}${poster.formats?.large?.url || poster.formats?.medium?.url}`}
            alt="poster"
            width={poster.formats?.large?.width || poster.formats?.medium?.width}
            height={poster.formats?.large?.height || poster.formats?.medium?.height}  
            className="rounded-[20px] mb-[1.5rem] w-full h-auto object-cover"
          />
